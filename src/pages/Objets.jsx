...
const Objets = () => {
    ...
    useEffect(() => {
        const fetchCards = async () => {
            try {
                const responseMaterials = await fetch(BACK_URI+"/material/getMaterials");
                const dataMaterials = await responseMaterials.json();

                setCardData(dataMaterials);
            } catch (err) {
            ...
            <div id="content">
                ...
                <div id="listingItems">
                    {currentCards.map((result) => (
                        <a href={`./Objet/${result.id}`} id="parentLight" key={result.id}>
                            <div>
                                <img src={`../updateApt/${result.name}.webp`} alt={result.name}
                                     style={{width: "100%"}}/>
